...
databaseSetUp.promise.then(function() {
	r.table('posts').filter(r.row('source').eq('techcrunch')).orderBy(r.desc('createdAt')).limit(1).
...
feedparser.on('readable', function() {
    ...
    recentPostLoaded.promise.then(function(lastPost) {
    	...
    	while (item = stream.read()) {
	        ...
	        var article = {
	        	title: item.title,
	        	description: descriptionText.substring(0, descriptionText.indexOf('Read More')).trim(),
	        	image: description('img').attr('src'),
	        	url: item.link,
    	...
    	if (posts.length) {
			r.table('posts').insert(posts).run(connection, function(err, result) {
			    ...
			    console.log('Added ' + posts.length + ' posts from TechCrunch')
			})
    	};
